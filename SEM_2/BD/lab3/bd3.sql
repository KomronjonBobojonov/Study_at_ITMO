...
CREATE OR REPLACE FUNCTION log_new_observation()
RETURNS TRIGGER AS 
$$
BEGIN
   
    INSERT INTO observationLogs (observation_id, details)
    VALUES (NEW.id, CONCAT('Новое наблюдение зафиксировано на месте: ', NEW.location));
    RETURN NEW;
...
CREATE TRIGGER after_insert_observation
AFTER INSERT ON observation
FOR EACH ROW
EXECUTE FUNCTION log_new_observation();
...
--     permission INT,
--     timeofshot TIMESTAMP,
--     frequency_camera INTEGER,
--     location VARCHAR(30),
--     probe_id INTEGER,
--     FOREIGN KEY (asteroid_id) REFERENCES asteroid(id),
